 ...
 * 
 */
public abstract class CompressionDecompressor implements IDecompressor {
	
	/**
	 * 
	 */
	public CompressionDecompressor() {
		super();
	}
	...
	protected File prepareOutputFile(final File archive,
		...
		// determine filename without the extension
		final String baseName = FilenameUtils.getBaseName(archive.getName());
		...
		// outputFile is a file given by the baseName of the archive in the output directory
		final File outputFile = new File(outputDirectory, baseName);
		
		// we do not override existing files
		if (outputFile.exists()) {
			throw new FileAlreadyExistsException("Target file '" + outputFile.getAbsolutePath() + "' already exists.");
		}
